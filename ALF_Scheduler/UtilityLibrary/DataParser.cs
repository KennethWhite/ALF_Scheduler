...
namespace ALF_Scheduler.Utilities {
    ...
    public static class DataParser
    {
        ...
        private static Excel.Range _row;
        ...
        /// </summary>
        public static Facility ParseFacility(Excel.Range excelRow) {
        ...
        public static Facility UpdateFacility(Facility fac, Excel.Range excelRow) {
            _facility = fac;
            _row = excelRow;
        ...
        {
            Inspection ret = new Inspection {InspectionDate = CreateDateTime(date)};
            if(!string.IsNullOrEmpty(code)){
                ret.Code= Code.getCodeByName(code);
            }
        ...
        {
            bool success = int.TryParse(beds, out int parsed);
        ...
        public static DateTime CreateDateTime(string date) {
            Regex regex = new Regex(@"^((\d{1,2}(-)\d{1,2}(-)\d{4})|(\d{1,2}(\/)\d{1,2}(\/)\d{4}))$");
